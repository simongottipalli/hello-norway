...
Next.js 16 (App Router, React 19, TypeScript 5), Tailwind CSS v4 with shadcn/ui (zinc theme), Express 5 backend, Prisma 7 ORM with PostgreSQL 16 (via `@prisma/adapter-pg`), OTP-based email auth with cookie sessions, Brevo for email, Pino for structured logging.
...
- `src/routes/` — Express routes (`otpRoutes.ts`; others migrated to tsoa)
- `src/controllers/` — tsoa controllers (`AuthController`, `OtpController`, `TaskController`, `OnboardingController`)
- `src/generated/routes.ts` + `src/generated/swagger.json` — auto-generated by tsoa (do not edit)
