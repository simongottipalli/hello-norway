...
describe("Task API", () => {
  ...
  describe("DELETE /api/tasks/:id", () => {
    ...
    let ownerDeleteUserId: string;
    let ownerDeleteUserEmail: string;
    const deleteTaskSlug = () => `delete-task-${Date.now()}-${Math.random().toString(16).slice(2)}`;

    beforeAll(async () => {
      ownerDeleteUserId = randomUUID();
      ownerDeleteUserEmail = generateUniqueTestEmail();
      await prisma.user.create({
        data: { id: ownerDeleteUserId, email: ownerDeleteUserEmail, name: "Delete Owner" },
      });
    });

    afterAll(async () => {
      await prisma.user.deleteMany({ where: { id: ownerDeleteUserId } }).catch(() => {});
    });

    beforeEach(async () => {
      // Create a fresh task owned by ownerDeleteUserId for each test
      authUserId = ownerDeleteUserId;
      authUserEmail = ownerDeleteUserEmail;
    ...
    it("should delete an owned task and return 200", async () => {
      authUserId = ownerDeleteUserId;
      authUserEmail = ownerDeleteUserEmail;

      const response = await request(app).delete(`/api/tasks/${deleteTaskId}`);

      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);

      // Verify it is gone
      const check = await request(app).get(`/api/tasks/${deleteTaskId}`);
      expect(check.status).toBe(404);
    });

    it("should return 403 when a different user attempts to delete the task", async () => {
      authUserId = randomUUID();
      authUserEmail = generateUniqueTestEmail();
